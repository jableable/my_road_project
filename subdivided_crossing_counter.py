...
def visualize_map(lat,lng,img_path,polygon_unshifted,final_linestrings,final_crossings,crossings):        
    ...
    #divide stored img into 4x4 grid and store this grid in tiles
    M = img.shape[0]//4
    N = img.shape[1]//4
    grid_x_coords = [i*M for i in range(5)]
    grid_y_coords = [(4-i)*N for i in range(5)]
    grid = []
    for i in range(4):
        for j in range(4):
            square = [(grid_x_coords[j],grid_y_coords[i+1]),(grid_x_coords[j],grid_y_coords[i]),(grid_x_coords[j+1],grid_y_coords[i]),(grid_x_coords[j+1],grid_y_coords[i+1])]
            grid.append(square)
